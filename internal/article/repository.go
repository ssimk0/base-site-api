...
func (r *repository) Find(id uint) (*Article, error) {
	article := Article{}
...
func (r *repository) FindAll(order string, offset int, limit int) ([]*Article, int, error) {
	var articles []*Article
	var count int

	if err := r.db.Set("gorm:auto_preload", true).Model(&Article{}).Order(order).Where("published = 1").Count(&count).Error; err != nil {
		return nil, 0, err
	}

	if err := r.db.Set("gorm:auto_preload", true).Model(&Article{}).Offset(offset).Limit(limit).Order(order).Where("published = 1").Find(&articles).Error; err != nil {
		return nil, 0, err
	}
...
// Update the article
func (r *repository) Update(article *Article, id uint) error {
	a, err := r.Find(id)
	if err != nil {
		return err
	}
...
func (r *repository) Store(article *Article, userID uint) (uint, error) {
